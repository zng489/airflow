...
def load_data_from_gcs_to_bq():
    ...
    gcs_uri = 'gs://gold_oppai/posts_comments/posts_comments.parquet'

    # Set the BigQuery dataset and table name
    dataset_id = 'big_query_oppai_data'
    table_id = 'table_posts_comments'
    try:
...
with DAG(
        ...
        dag_id = 'bq_posts_comments',
        description = 'bq_posts_comments',
        start_date = datetime(2023, 11, 23),
        schedule_interval='@daily'
    ) as dag:
        start_task = DummyOperator(task_id='start_lnd', dag=dag)
        task1 = PythonOperator(task_id = 'bq_posts_comments',python_callable = load_data_from_gcs_to_bq)
